...
public:
	...
	Image(const int width,const int height, void* buffer, int sizeOfColor, const std::string &fileName = std::string("Unnamed"));
	...
	/// @param threadman A pointer to a threadmanager. May be null. Used to multithread this method
	int save(const std::string &fileName, a7az0th::ThreadManager *threadman) const;
...
private:
	...
	int own;          ///< True of the image buffer is owned by this object and should be freed upon deallocation
	int colorSize;    ///< Usually a color is the size of 3 floats, but when an external buffer is provided a different size may be provided
